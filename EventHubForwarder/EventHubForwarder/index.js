...
module.exports = async function main(context, eventHubMessages) {
  ...
  if (typeof eventHubMessages === 'string') {
    logs = eventHubMessages.trim().split('\n');
  } else if (Buffer.isBuffer(eventHubMessages)) {
    logs = eventHubMessages.toString('utf8').trim().split('\n');
  } else if (!Array.isArray(eventHubMessages)) {
    logs = JSON.stringify(eventHubMessages).trim().split('\n');
  } else {
    logs = eventHubMessages;
  }
